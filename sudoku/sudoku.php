...
function sudoku_display() {
    ...
    .bot-message.typing-indicator p::after {
        content: '...';
    ...
    /* Scrollbar styling for chat messages (optional) */
                ...
                <div class="accordion" id="faqAccordion">
                    ...
                    <div class="accordion-item">
                        ...
                        <div id="faqCollapse3" class="accordion-collapse collapse" aria-labelledby="faqHeading3" data-bs-parent="#faqAccordion">
                            <div class="accordion-body">
                                The FREE Sudoku challenge will start on 16th of June 2025.<br>
                                The Premium preparatory course will start a week earlier, i.e., 9th June 2025.
                            </div>
                        </div>
                    </div>
                    <div class="accordion-item">
        ...
        <div id="chat-popup" class="chat-popup">
            <div class="chat-header">
                ...
                <div class="chat-header-buttons">
                    <button id="clear-chat" class="clear-chat-btn" title="Clear Chat">
            ...
            <div id="chat-messages" class="chat-messages">
                <div class="message bot-message">
                    <p>Hello! I'm here to help you with the Kaizen Sudoku Challenge. Ask me anything about our programs, benefits, or how to get started!</p>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const backendUrl = 'https://kaizen-pq9y.onrender.com/api/chat'; // Ensure this matches your backend port
        ...
        function toggleChatPopup() {
            if (chatPopup.classList.contains('show')) {
                ...
                chatIcon.style.display = 'flex'; // Show icon when closing popup
            } else {
                chatPopup.classList.add('show');
                chatIcon.style.display = 'none'; // Hide icon when opening popup
                chatInput.focus();
            }
        }    function addMessageToChat(message, sender, messageType = null) {
            const messageElement = document.createElement('div');
            messageElement.classList.add('message', sender === 'user' ? 'user-message' : 'bot-message');
            
            // Add special styling for contact request messages
            if (messageType === 'contact_request') {
                messageElement.classList.add('contact-request');
            }
            ...
            // Add to conversation history (skip the initial bot greeting)
            if (conversationHistory.length > 0 || sender === 'user') {
        ...
        function showTypingIndicator() {
            ...
            const pElement = document.createElement('p');
            pElement.textContent = 'Assistant is typing'; // CSS will add '...'
            typingIndicator.appendChild(pElement);
        ...
        function removeTypingIndicator() {
            ...
            try {
                ...
                if (!response.ok) {
                    const errorData = await response.json().catch(() => ({ error: "An unknown error occurred", details: null }));
                    console.error('Error from backend:', response.status, errorData);
                    let errorMessage = "Error: " + (errorData.error || response.statusText);
                    if (errorData.details && errorData.details.promptFeedback && errorData.details.promptFeedback.blockReason) {
                        errorMessage += " (Reason: " + errorData.details.promptFeedback.blockReason + ")";
                ...
                // Handle multiple messages from backend
                if (data.messages && Array.isArray(data.messages)) {
                    // Add each message as a separate bot message
                    data.messages.forEach((msg, index) => {
                        setTimeout(() => {
                            addMessageToChat(msg.content, 'bot', msg.type);
                        }, index * 1000); // Add 1 second delay between messages
                    });
                } else if (data.reply) {
                    // Fallback for single reply format
                ...
                addMessageToChat('Sorry, I couldn\'t connect to the server. Please try again later.', 'bot');        }
        }

        function clearConversation() {
            // Clear the visual chat
            chatMessages.innerHTML = '<div class="message bot-message"><p>Hello! How can I help you today? Ask me anything about our services!</p></div>';
